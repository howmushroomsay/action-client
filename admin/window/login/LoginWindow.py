...
from view.Ui_LoginWindow import Ui_Form
...
class LoginWindow(AcrylicWindow, Ui_Form):
    ...
    def initConfig(self):
        config = yaml.load(open('./window/login/config/ServerConfig.yaml', 'r'), 
                           Loader=yaml.FullLoader)
        self.host = config["server"]["host"]
        self.port = config["server"]["port"]
        config = yaml.load(open('./window/login/config/ClientConfig.yaml', 'r'), 
                           Loader=yaml.FullLoader)
    ...
    def signIn(self):
        ...
        if (response["code"] == 1):
            ...
            with open('./window/login/config/ClientConfig.yaml','r') as f:
                config = yaml.load(f, Loader=yaml.FullLoader)
                config["user"]["token"] = "\"" + token + "\""
                # yaml.(mapping=2, sequence=4, offset=2)  
            with open('./window/login/config/ClientConfig.yaml','w') as f:                
                yaml.dump(config, f)
